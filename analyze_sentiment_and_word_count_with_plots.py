import pandas as pd
from textblob import TextBlob  # Importujemy TextBlob
...
sns.barplot(x=sentiment_by_tone.index, y=sentiment_by_tone.values, palette='coolwarm', hue=sentiment_by_tone.index)
...
plt.figure(figsize=(8, 6))
sns.barplot(x=word_count_by_tone.index, y=word_count_by_tone.values, palette='Blues', hue=word_count_by_tone.index)
...
df.to_csv('modified_generated_data_with_analysis.csv', index=False)
print('CSV file with results saved as modified_generated_data_with_analysis.csv')